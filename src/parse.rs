...
impl Parser {
    ...
    fn parse_function_dec(&mut self) -> Result<ParseTreeNode, String> {
        ...
        // params MUST be inserted into scope before parsing block
        {
            ...
            while i < param_children.len() {
                ...
                self.insert_in_scope(name, VarEntry { var_type: type_val, _var_value: Expr::Int(0)});
                i += 2;
            }
        }
    ...
    fn parse_array_index_expr(&mut self, base_node: ParseTreeNode) -> Result<ParseTreeNode, String> {
        let left_bracket = ParseTreeNode {
            symbol: ParseTreeSymbol::ParseTreeSymbolTerminalLeftBracket,
            children: vec![],
            value: None,
        ...
        Ok(ParseTreeNode {
            ...
            children: vec![base_node, left_bracket, index_expr, right_bracket],
            value: None,
        })
    }
    ...
    // Unary → ("-") Unary | Primary
    fn parse_unary(&mut self) -> Result<ParseTreeNode, String> {
        if let Some(token) = self.current() {
            if token.token_type == TokenType::TokenTypeMinus {
                let op_terminal = ParseTreeNode {
                    symbol: ParseTreeSymbol::ParseTreeSymbolTerminalMinus,
                    children: Vec::new(),
                    value: None,
                };
                self.consume();
                ...
                return Ok(ParseTreeNode {
                    symbol: ParseTreeSymbol::ParseTreeSymbolNodeUnary,
                    children: vec![op_terminal, right],
                    value: None,
                });
            }
        }
        ...
        match token.token_type {
            ...
            TokenType::TokenTypeIdentifier => {
                ...
                if next_type == Some(TokenType::TokenTypeLeftParen) {
                    ...
                    // Possible indexing after function call: foo()[0]
                    while self.current().map(|t| t.token_type) == Some(TokenType::TokenTypeLeftBracket) {
                        node = self.parse_array_index_expr(node)?;
                        node = ParseTreeNode {
                            symbol: ParseTreeSymbol::ParseTreeSymbolNodePrimary,
                            children: vec![node],
                            value: None,
            ...
            TokenType::TokenTypeLeftBracket => {
                ...
                // Possible indexing after array literal: [1, 2, 3][0]
                while self.current().map(|t| t.token_type) == Some(TokenType::TokenTypeLeftBracket) {
                    node = self.parse_array_index_expr(node)?;
                    node = ParseTreeNode {
                        symbol: ParseTreeSymbol::ParseTreeSymbolNodePrimary,
    ...
    fn get_expr_type(&self, expr: &Expr) -> Type {
        match expr {
            ...
            Expr::UnaryOp { op: _, expr } => self.get_expr_type(expr),
    ...
    fn types_compatible(&self, expected: &Type, actual: &Type, actual_expr: &Expr) -> bool {
        if expected == actual {
            return true;
        }

        // Special case: empty array literal can match any array type
        if let Expr::ArrayLiteral(elements) = actual_expr {
            if elements.is_empty() {
                if let Type::Array(_) = expected {
                    if let Type::Array(_) = actual {
                         return true;
                    }
                }
            }
        ...
        match parse_tree.symbol {
            ParseTreeSymbol::ParseTreeSymbolNodeEntryPoint => {
                ...
                if self.functions.get("main").is_none() {
                    panic!("Fatal: Program must contain a 'main' function.");
                }
            ...
            ParseTreeSymbol::ParseTreeSymbolNodeVariableDeclaration => {
                ...
                if !self.types_compatible(&declared_type, &actual_type, &value_expr) {
                     panic!("TypeChecker: Mismatched types in declaration of '{}'. Expected {:?}, found {:?}", name, declared_type, actual_type);
                }
 
                self.insert_in_scope(name.clone(), VarEntry { var_type: declared_type.clone(), _var_value: value_expr.clone() });
            ...
            ParseTreeSymbol::ParseTreeSymbolNodeVariableAssignment => {
                ...
                if target_node.symbol == ParseTreeSymbol::ParseTreeSymbolTerminalIdentifier {
                    ...
                    if !self.types_compatible(&var_type, &actual_type, &value_expr) {
                    ...
                    if !self.types_compatible(&element_type, &actual_type, &value_expr) {
            ...
            ParseTreeSymbol::ParseTreeSymbolNodeFor => {
                ...
                self.push_scope();
                self.insert_in_scope(iterator_name.clone(), VarEntry { var_type: Type::I32S, _var_value: Expr::Int(0) });
            ...
            ParseTreeSymbol::ParseTreeSymbolNodeReturn => {
                ...
                if parse_tree.children.len() == 2 {
                    ...
                    if let Some(expected_type) = self.return_type_stack.last() {
                         ...
                         if !self.types_compatible(expected_type, &actual_type, &expr) {
    ...
    fn build_primary(&mut self, node: &ParseTreeNode) -> Expr {
        ...
        self.build_inner_primary(&node.children[0])
    }

    fn build_inner_primary(&mut self, node: &ParseTreeNode) -> Expr {
        match node.symbol {
            ParseTreeSymbol::ParseTreeSymbolNodePrimary => {
                // This case handles when a primary expression itself contains another primary expression,
                // e.g., in array indexing where the base is a primary.
                self.build_primary(node)
            }
            ParseTreeSymbol::ParseTreeSymbolTerminalIntegerLiteral => {
                let value = node.value.as_ref().unwrap().parse::<i32>().unwrap();
                Expr::Int(value)
            }
            ParseTreeSymbol::ParseTreeSymbolTerminalFloatLiteral => {
                let value = node.value.as_ref().unwrap().parse::<f32>().unwrap();
                Expr::Float(value)
            }
            ParseTreeSymbol::ParseTreeSymbolTerminalBooleanLiteral => {
                let value = node.value.as_ref().unwrap().parse::<bool>().unwrap();
                Expr::Bool(value)
            }
            ParseTreeSymbol::ParseTreeSymbolTerminalIdentifier => {
                let ident = node.value.as_ref().unwrap().clone();
            ...
            ParseTreeSymbol::ParseTreeSymbolNodeFunctionCall => {
                let name = node.children[0].value.as_ref().unwrap().clone();
                let mut args = Vec::new();
            ...
            ParseTreeSymbol::ParseTreeSymbolTerminalCharLiteral => {
                let value = node.value.as_ref().unwrap().chars().next().unwrap();
                Expr::Char(value)
            }
            ...
            ParseTreeSymbol::ParseTreeSymbolNodeArrayIndex => {
                // The first child of ParseTreeSymbolNodeArrayIndex is the base expression (e.g., an identifier or another array index)
                let base_expr = self.build_inner_primary(&node.children[0]);
                let index_expr = self.build_expr(&node.children[2]);
                Expr::ArrayIndex {
                    array: Box::new(base_expr),
                    index: Box::new(index_expr),
                }
            }
            _ => panic!("Unsupported expression type in primary: {:?}", node.symbol),
        }
    }

    fn build_unary(&mut self, node: &ParseTreeNode) -> Expr {
        if node.children.len() == 2 {
            // Unary op: [op, expr]
            let expr = self.build_unary(&node.children[1]);
            return Expr::UnaryOp {
    ...
    fn build_expr(&mut self, node: &ParseTreeNode) -> Expr {
        ...
        match child.symbol {
            ...
            ParseTreeSymbol::ParseTreeSymbolNodeUnary => self.build_unary(child),
